package org.csystem.app.samples.simulation.randomobjectgenerator;

import org.csystem.util.ArrayUtil;
import org.csystem.util.datetime.Date;
import org.csystem.util.datetime.Time;

public class RandomObjectGeneratorSimulation {
    private RandomObjectGeneratorSimulation()
    {
    }

    public static void run()
    {
        RandomObjectArrayGenerator generator = new RandomObjectArrayGenerator(10);

        generator.run();

        for (Object object : generator.getObjects()) {
            System.out.printf("Dinamik tür ismi:%s%n", object.getClass().getName());

            if (object instanceof String) {
                String s = (String)object;

                System.out.println(s.toUpperCase());
            }
            else if (object instanceof Integer) {
                int val = (int)object;

                System.out.printf("%d * %d = %d%n", val , val, val * val);
            }
            else if (object instanceof Date) {
                Date date = (Date)object;
                System.out.println(date.toLongDateStringTR());
            }
            else if (object instanceof Time) {
                Time time = (Time)object;

                System.out.println(time.toLongTimeString());
            }
            else if (object instanceof Double) {
                double val = (double)object;

                System.out.printf("val = %f%n", val);
            }
            else if (object instanceof Character) {
                char ch = (char)object;

                System.out.printf("ch = %c%n", ch);
            }
            else if (object instanceof int[]) {
                int [] a = (int []) object;

                ArrayUtil.display(2, a);
            }
            System.out.println("//////////////////////");
        }
    }
}
